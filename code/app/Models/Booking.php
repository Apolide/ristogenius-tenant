...
class Booking extends Model
{
    ...
    public function isWalkIn(): bool { return $this->source === 'walk-in'; }
    public function tables(): BelongsToMany { return $this->belongsToMany(RoomTable::class, 'booking_room_table'); }
    public function histories(): HasMany { return $this->hasMany(BookingHistory::class)->latest(); }

    public function recordHistory(string $event, ?string $description = null, array $changes = []): void
    {
        $this->histories()->create([
            'event' => $event,
            'actor' => auth()->user()?->name ?? ($this->source === 'backoffice' ? 'Backoffice' : ucfirst($this->source)),
    ...
    protected static function booted(): void
    {
        ...
        static::updated(function (Booking $booking): void {
            ...
            if ($changes !== []) $booking->recordHistory(array_key_exists('status', $changes) ? 'status_changed' : 'updated', 'Prenotazione modificata', $changes);
        });
