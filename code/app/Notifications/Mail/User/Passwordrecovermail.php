...
class Passwordrecovermail extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        $mailMessage->subject(__($this->payload['content_name']." - ".$this->payload['display_name']." reimposta la tua password."));
        $mailMessage->greeting(__("Ciao :display_name", ["display_name" => $this->payload['display_name']]));
        $mailMessage->line("Tu o qualcuno per te ha richiesto di reimpostare la password del tuo account su ".$this->payload['content_name'].". Se non sei stato tu ad effettuare la richiesta ignora semplicamente questa e-mail.");
