    ...
    <section class="box"><div class="box-header"><h2 class="box-title">{{ __('public_bookings.edit_title') }}</h2></div><div class="box-body">
        ...
        <form wire:submit="save">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div><label class="form-label">{{ __('bookings.form.date') }}</label><input type="date" min="{{ now()->toDateString() }}" wire:model.live="booking_date" class="form-control cursor-pointer dark:[color-scheme:dark]" onclick="if (this.showPicker) this.showPicker()" required></div>
                ...
                <div><label class="form-label">{{ __('bookings.form.people') }}</label><input type="number" min="1" wire:model.live.debounce.300ms="pax" class="form-control" required></div>
            </div>
