<div class="content booking-room-page"><div wire:poll.30s class="main-content booking-room-main">
    <style>
        ...
        :is(.dark .booking-card){background:#1f2937;border-color:#475569;color:#e5e7eb}
        :is(.dark .booking-card.highlighted){background:#172554;border-color:#60a5fa}
        :is(.dark .booking-table-badge){color:#f8fafc!important}
        :is(.dark .room-layout){background:#1f2937;border-color:#475569}
        :is(.dark .room-inner){background:#1f2937}
        :is(.dark .booking-scroll-shadow){color:#cbd5e1}
        :is(.dark .booking-scroll-shadow svg){background:rgba(31,41,55,.94)}
        :is(.dark .booking-scroll-shadow-top){background:linear-gradient(to bottom,#1f2937 0,rgba(31,41,55,.94) 45%,rgba(31,41,55,0) 100%)}
        :is(.dark .booking-scroll-shadow-bottom){background:linear-gradient(to top,#1f2937 0,rgba(31,41,55,.94) 45%,rgba(31,41,55,0) 100%)}
    ...
    <div
        ...
        wire:key="room-map-{{ $roomId }}-{{ $date }}-{{ $meal }}-{{ md5(json_encode(collect($roomBookings)->map(fn (array $booking): array => [$booking['id'], $booking['status']])->all())) }}"
            ...
            <aside class="booking-sidebar" :class="{'hidden-panel':!sidebarOpen}" aria-label="Prenotazioni">
                ...
                <div class="booking-scroll-wrap">
                    ...
                    <div x-ref="bookingScroller" class="booking-list-scroller max-h-[540px] overflow-y-auto pr-1" @scroll.passive="updateBookingScrollShadows()" @resize.window.debounce.100ms="updateBookingScrollShadows()">
                        ...
                        @endphp
                        <article id="booking-list-{{ $booking->id }}" class="booking-card" :class="{'highlighted':highlightedBookingId==='{{ $booking->id }}','is-drag-origin':dragging?.bookingId==='{{ $booking->id }}'&&dragging?.fromTableId===null}" @click.capture="if(Date.now()<suppressClicksUntil){$event.preventDefault();$event.stopPropagation()}">
                            <div class="flex justify-between items-start gap-2">
                                ...
                                <strong class="booking-drag-handle whitespace-nowrap" title="Trascina per assegnare a un tavolo" @pointerdown="onListBookingPointerDown('{{ $booking->id }}',$event)"><i class="las la-clock"></i> {{ substr($booking->booking_time,0,5) }}</strong>
                            </div>
                            ...
                            <div class="border-t mt-2 pt-2"><div class="flex flex-wrap items-center gap-1 text-xs"><i class="las la-chair"></i>@forelse($booking->tables as $table)<span class="badge booking-table-badge bg-secondary/10 text-black">{{ $table->name }}</span>@empty<span class="text-danger">Tavolo non assegnato</span>@endforelse</div>
            ...
            <section class="room-layout" aria-label="Mappa sala">
                <div class="room-inner" :style="roomInnerStyle()">
                    <template x-for="table in tables" :key="table.id"><div :class="tableCssClasses(table)" :style="tableStyle(table)" :data-drop-table-id="table.id" @click="onTableClick(table.id,$event)">
                        ...
                        <div class="table-bookings" x-show="table.bookings.length"><template x-for="booking in table.bookings" :key="`${table.id}-${booking.id}`"><div class="table-booking-row" :class="{'is-drag-origin':isDraggedBooking(table.id,booking.id)}" :title="fullBookingRowLabel(booking)" @pointerdown.stop="onBookingPointerDown(table.id,booking.id,$event)" @click.stop="onBookingRowClick(booking.id)" x-text="bookingRowLabel(booking)"></div></template></div>
                    </div></template>
                </div>
            </section>
        </div>
    ...
    @if($modal)
        ...
        <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4" wire:click.self="closeModal"><div class="bg-white dark:bg-bodybg rounded-lg shadow-xl p-5 w-full max-w-md">
            ...
            @else
                @php $confirmationKey = str_replace('-', '_', $modal); @endphp
                <h2 class="text-xl font-bold mb-3">{{ __("bookings.actions.confirmations.{$confirmationKey}.title") }}</h2><p class="mb-4">{{ __("bookings.actions.confirmations.{$confirmationKey}.prompt") }} <strong>{{ $modalBooking?->customer?->display_name ?: 'Walk In' }}</strong>?</p>
...
window.roomBookingDnD = function(config) { config=config||{}; return {
    ...
    fullBookingRowLabel(booking){return `${booking.time} (${booking.pax}) ${booking.customer}`},
    bookingRowLabel(booking){let label=this.fullBookingRowLabel(booking);return label.length>21?`${label.slice(0,20)}…`:label},
