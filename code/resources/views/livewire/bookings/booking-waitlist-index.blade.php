                ...
                <form wire:submit="createWaitlistBooking" class="box border mb-5">
                    ...
                    <div class="box-body !py-3">
                        <div class="grid grid-cols-2 sm:grid-cols-1 gap-x-3 gap-y-2">
                            ...
                            <div><label class="form-label !mb-1">{{ __('bookings.form.phone') }}</label><div class="flex gap-2"><select wire:model="phone_region" class="form-control max-w-[220px]" autocomplete="tel-country-code">@foreach($countries as $country)<option value="{{ $country['region'] }}">{{ $country['flag'] }} {{ $country['prefix'] }}</option>@endforeach</select><input type="tel" wire:model="phone" class="form-control" inputmode="tel" autocomplete="tel-national" maxlength="25"></div>@error('phone')<p class="text-danger text-sm">{{ $message }}</p>@enderror @error('phone_region')<p class="text-danger text-sm">{{ $message }}</p>@enderror</div>
