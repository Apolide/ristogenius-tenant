    ...
    <div class="box"><div class="box-body">
        ...
        <form wire:submit="save" oninput="const phone = this.elements.phone; const email = this.elements.email; phone.required = email.value.trim() === ''; email.required = phone.value.trim() === ''; this.elements.phone_prefix.required = phone.value.trim() !== '';">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
                ...
                <div><label class="block text-sm font-medium">{{ __('bookings.form.status') }}</label>@if($booking)<select wire:model="status" class="form-control">@foreach($statuses as $value => $status)<option value="{{ $value }}">{{ __('bookings.statuses.'.$value) }}</option>@endforeach</select>@else<div class="form-control bg-light cursor-not-allowed" aria-readonly="true">{{ __('bookings.statuses.accepted') }}</div>@endif</div>
            </div>
