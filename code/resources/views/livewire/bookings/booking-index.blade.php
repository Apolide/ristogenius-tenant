<div class="content"><div class="main-content">
    ...
    <div class="box"><div wire:poll.15s class="box-body">
        <div class="grid grid-cols-3 gap-3">
            <div class="flex justify-start"><button wire:click="previousDay" class="ti-btn ti-btn-primary me-[0.375rem]"><i class="las la-arrow-left text-3xl"></i><span class="hidden md:block">Indietro</span></button></div>
            <div class="flex justify-center gap-2">
                ...
                <button type="button" wire:click="selectToday" class="ti-btn ti-btn-primary" title="Vai a oggi">Oggi</button>
            </div>
            <div class="flex justify-end"><button wire:click="nextDay" class="ti-btn ti-btn-primary me-[0.375rem]"><span class="hidden md:block">Avanti</span><i class="las la-arrow-right text-3xl"></i></button></div>
        </div>
        ...
        <div x-data="{ open: $wire.entangle('timeslotAccordionOpen').live }" class="mb-1 mt-5">
            <div class="overflow-hidden bg-white border -mt-px first:rounded-t-sm last:rounded-b-sm dark:bg-bodybg dark:border-white/10">
                <button type="button" class="bg-primary/10 text-primary group py-4 px-5 inline-flex items-center justify-between gap-x-3 w-full text-start transition hover:text-success dark:text-gray-200 dark:hover:text-white/80" @click="open = !open" :aria-expanded="open.toString()">
                    <span class="inline-flex items-center gap-2"><i class="las la-clock text-xl"></i> Vedi prenotazioni per fascia oraria</span>
                ...
                <div x-show="open" x-transition x-cloak class="w-full overflow-hidden">
                    ...
                    <div class="box-body"><div class="table-responsive"><table class="ti-custom-table w-full"><thead><tr><th>Fascia</th><th>Prenotati</th><th>Prenotazioni</th></tr></thead><tbody>
                        @forelse($timeslotStats as $slot)
                            ...
                            @endif
                            @empty<tr><td colspan="3" class="text-center">Ristorante chiuso in questa data.</td></tr>
                        @endforelse
                    </tbody></table></div></div>
                </div>
            </div>
        ...
        <div class="table-responsive mt-4"><table class="table table-bordered whitespace-nowrap min-w-full"><thead><tr><th>Cliente / prenotazione</th><th>Tavoli / permanenza</th></tr></thead><tbody>
        @forelse($bookings as $booking) @php($status = $statuses[$booking->status] ?? ['label'=>$booking->status,'color'=>'secondary'])
            <tr><td><p>@if($booking->customer)<a href="{{ route('customers.show', $booking->customer) }}" class="text-primary"><b>{{ $booking->customer->display_name ?: trim($booking->customer->firstname.' '.$booking->customer->lastname) }}</b></a>@else<b><i class="las la-walking"></i> Walk In</b>@endif @php($language = app(\App\Services\CustomerLanguageService::class)->meta($booking->language))<small title="{{ $language['label'] }}">{{ $language['flag'] }} {{ strtoupper($booking->language) }}</small></p><p><i class="las la-users"></i> {{ $booking->pax }} <i class="las la-clock"></i> {{ substr($booking->booking_time,0,5) }} <span class="badge bg-{{ $status['color'] }}/10 text-{{ $status['color'] }}">{{ $status['label'] }}</span></p>
                ...
                <td>@foreach($booking->tables as $table)<span class="badge bg-secondary/10 text-black">{{ $table->name }}</span> @endforeach @if($booking->status === 'seated') @php($p=$this->progress($booking,$maxSitting))<div class="progress mt-3"><div class="progress-bar {{ $p < 60 ? '!bg-success' : ($p < 85 ? '!bg-warning' : '!bg-danger') }}" style="width:{{ $p }}%"></div></div><small>{{ $p }}%</small>@endif</td></tr>
        @empty<tr><td colspan="2" class="text-center py-5">Nessuna prenotazione per la selezione corrente.</td></tr>@endforelse
        </tbody></table></div>
    </div></div>
    @if($modal === 'tables')
